...
float get_Distance(Eigen::Matrix4f &key_transform, KeyPoint &p1_key, KeyPoint &p2_scan, const float resolution = 0.01f)
{
	...
	KeyPoint &p2_scan为扫描点云一个关键点
	*/


	float x_transform = p1_key.Key_coordinate.x - p2_scan.Key_coordinate.x;		//p2是扫面点云关键点
	float y_transform = p1_key.Key_coordinate.y - p2_scan.Key_coordinate.y;
	float z_transform = p1_key.Key_coordinate.z - p2_scan.Key_coordinate.z;
	Eigen::Matrix4f transform_mat = Eigen::Matrix4f::Identity();	//平移矩阵
	transform_mat(0, 3) = x_transform;
	transform_mat(1, 3) = y_transform;
	transform_mat(2, 3) = z_transform;
	pcl::PointCloud<pcl::PointXYZ>::Ptr transformed_cloud(new pcl::PointCloud<pcl::PointXYZ>());
	pcl::transformPointCloud(*p1_key.Occupiedgrid.cloud, *transformed_cloud, transform_mat);		//平移变换，使两个关键点重合
	...
	Eigen::Matrix4f transform_1 = Eigen::Matrix4f::Identity();	//平移矩阵1：使占据网格平移到原点，在原点进行旋转
	transform_1(0, 3) = -p1_key.Key_coordinate.x;
	transform_1(1, 3) = -p1_key.Key_coordinate.y;
	...
	Eigen::Matrix4f transform_3 = Eigen::Matrix4f::Identity();	//平移矩阵，是占据网格在旋转后从原点
	transform_3(0, 3) = p1_key.Key_coordinate.x;
	transform_3(1, 3) = p1_key.Key_coordinate.y;
	...
	for (int i = 0; i < 36; i++)
	{
		...
		pcl::octree::OctreePointCloud<pcl::PointXYZ>octree_temp(resolution);
		octree_temp.defineBoundingBox(p1_key.Border[0], p1_key.Border[1], p1_key.Border[2], p1_key.Border[3], p1_key.Border[4], p1_key.Border[5]);
		octree_temp.setInputCloud(transformed_cloud);
		octree_temp.addPointsFromInputCloud();
		std::vector<pcl::PointXYZ, Eigen::aligned_allocator<pcl::PointXYZ>> pointGrid;
		int num_center = octree_temp.getOccupiedVoxelCenters(pointGrid);			//得到旋转后存在点云体素的中心 存放在pointGrid中
	...
	Eigen::Matrix4f transform3 = Eigen::Matrix4f::Identity();	//尺度缩放
	float change_size = p1_key.Key_coordinate.z / p2_scan.Key_coordinate.z;
	transform3(0, 0) = change_size;
	transform3(1, 1) = change_size;
	transform3(2, 2) = change_size;
